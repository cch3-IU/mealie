...
- **This is the `daycare` fork:** it adds a household-inventory/meal-planning overlay backed by a
  separate sidecar service (`mealie-daycare-processor`, a sibling repo), reached same-origin at
  `/api/daycare/v1/*`. Every fork-specific file (new or upstream-modified) is tracked in
  `overlay/README.md`, phase by phase, with live-verification notes — read it before touching
  anything under `frontend/app/components/Domain/Daycare/` or `mealie-daycare-processor`'s API
  surface. Never guess the sidecar's schemas/routes; its `api/schemas.py`/`api/routes/` are the
  source of truth, and `frontend/app/lib/api/types/daycare.ts` is a **hand-maintained** mirror of
  them (not generated by `task dev:generate`, which only walks `mealie/schema/*`).
- **`pnpm` via corepack is broken in this checkout:** `frontend/package.json`'s
  `devEngines.packageManager` makes the corepack `pnpm` shim fail with `Invalid package manager
  specification`. Use `npx --yes pnpm@latest <command>` instead.
...
- **`frontend/app/composables/api/api-client.ts`'s `delete<T>(url, config)`:** calls axios's own
  ...
  same 3-arg helper those use silently drops the real `config` (including custom headers) into a
  nonexistent third parameter; this bit a real caller (`DaycareAPI.deleteLot`'s Idempotency-Key)
  before being fixed. Keep `delete` as its own two-argument implementation.

## Key Files to Reference

- `Taskfile.yml` - All development commands and workflows
- `overlay/README.md` - This fork's patch surface over upstream Mealie, phase by phase (daycare feature history, live-verification notes, dev/staging harness usage)
