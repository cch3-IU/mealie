...
**Live-verified** (disposable copy of the sidecar's staging harness, cloned `--shared` into `.scratch/mdp-sidecar/` per the lane brief, built from `fm/mdp-ingredient-writeback` at `c5c4019` — identical to `main` at verification time): with `PROCESSING_WORKER_ENABLED=false` staging defaults, `GET /recipes/{slug}/daycare` 500s (`internal_error`) for a recipe still `pending` in the processing queue rather than 404ing (a pre-existing sidecar behavior unrelated to this phase, worth a follow-up); running `POST /processing/poll` a few times past the 120s quiet period classified the fixture recipes and the panel then rendered normally. Confirmed against a classified recipe: the per-recipe switch (off by default) and its hint render in the edit form and round-trip through `PUT .../daycare`; the household-wide switch and hint render in Daycare Settings' Automation card and `PUT /settings` succeeds with the field included (no 422, confirming the field can be sent safely against a sidecar build that doesn't yet know about it); "Preview cleaned ingredients" opens the dialog and renders a clean inline "Not Found" (via the existing `DaycareErrorState`, no crash, no console error) against the current sidecar's 404 on the not-yet-implemented preview route — this is the "degrades cleanly when the endpoints are absent" check. **Not** live-verified: any of the actual preview/apply/undo response rendering, the `can_apply` gating logic, or the four 409 codes, since no sidecar build with the real routes existed during this phase — those paths are covered only by `RecipeIngredientWritebackDialog.test.ts`'s fixtures, which were constructed from the frozen contract's prose and have not been checked against a live response. Re-verify those paths against a sidecar build carrying the real routes before trusting this dialog's behavior in production.
