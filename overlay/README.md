...
- `frontend/app/components/Domain/Daycare/DaycareUnlockDialog.vue` (+ test) — new dialog, modeled on `DaycarePrepCompletionDialog.vue`'s phase-driven shape (`loading` / `preview` / `not-available` / `submitting` / `receipt` / `error`). Opens straight to `GET /weeks/{week}/unlock-preview`; a safe plan (`preview.safe`) enables a single Confirm sending `{reason: null, force: false}`. An unsafe plan shows the sidecar's `reasons`, requires a non-empty reason textarea plus an acknowledgement checkbox before Confirm enables, then sends `{reason, force: true}`. A `409 unlock_unsafe` on confirm (the force:false race where the plan was safe at preview-load but not at submit) re-reads the fresh plan out of the error's `details` via `unlockPlanFromError()` (new export in `use-daycare.ts`, mirrors the existing `shoppingBlockers()`/`committedAtFromError()` pattern) and loops back to the same preview step rather than erroring — and mints a fresh Idempotency-Key for the retry, since a force:false→true payload change is a distinct logical mutation, not a replay (same reasoning as `updateLot`'s "a corrected resubmission is a distinct mutation" note above). A bare `404`/`405` shows "isn't available yet" (mirrors `DaycareLotEditDialog`'s degrade path) rather than a generic error. On success, shows the receipt with a "regenerate these weeks" prompt naming `downstream_weeks_marked_stale`.
...
- `frontend/app/lib/api/types/daycare.ts` — added `UnlockAvailability` (`WeekResponse.unlock?`, optional since an older sidecar predates the feature — the dashboard treats a missing/falsy `available` as "hide the action", never an error), `UnlockPlan`/`UnlockPreview`, `UnlockRequest`, `UnlockReceipt`. **Two unconfirmed-schema assumptions, both defended against in the UI rather than trusted**: (1) `created_lots`/`consumed_lots_restored` are typed as loose `Record<string, unknown>[]` (mirrors `UndoResult.restored_source_lots`'s existing precedent) since the brief's contract only pins down `reservations_released`'s shape — the dialog reads an optional `portions` field off each entry defensively (`totalPortions()`, sums only when every entry carries a numeric `portions`, else falls back to a bare count) rather than assuming a schema that isn't confirmed; (2) the unlock receipt's exact shape is nowhere specified in the brief, so `UnlockReceipt` is guessed to echo `UnlockPlan`'s fields (what will happen → what happened) plus `week_start`/`unlocked_at`/`reason`/`forced`, with **every plan-shaped field optional** — the receipt phase renders a "detail unavailable" note (mirrors `CommitReceipt.completion_preview?`/`DaycarePrepReceipt`'s existing null-summary fallback) rather than crashing if the real receipt omits them. Both assumptions need reconciling against the sidecar's actual `schemas.py` once `fm/mdp-unlock-week-endpoint` lands real code — as of this phase's authorship that branch is still byte-identical to the sidecar's `main` (confirmed via `git diff main --stat`), so there is no live schema to check against yet.
...
- `frontend/app/tests/stub-vuetify.ts` — added `VTextarea` and `VCheckbox` stubs (same input/emit pattern as the existing `VTextField`/`VSwitch` stubs), needed by `DaycareUnlockDialog.test.ts`'s unsafe-plan reason/acknowledgement fields.
- `frontend/app/lang/messages/en-US.json` (upstream file, modified) — adds the `daycare.plan.unlock-*` keys (button/dialog/receipt copy, the pluralized lots-removed/restored strings with and without a portions figure, reason/acknowledgement labels, not-available/detail-unavailable fallbacks). Already covered by the generic Phase 6 table entry below for this file; called out here since this is the phase that added these specific keys.

**Live verification status:** confirmed against a disposable copy of the staging harness (`dev/staging/`, cloned `--shared` into `.scratch/mealie-daycare-processor` per the lane brief, on the sidecar's `main` branch — `fm/mdp-unlock-week-endpoint` was confirmed byte-identical to `main` at authorship time via `git diff main --stat`, so this run used `main` directly). Seeded the 11 fixture recipes, ran a full + classification processing poll, set `daycare_portions_per_batch` on all 10 classified recipes and used the sidecar's `PUT /recipes/{slug}/daycare` `classification.uses` override to cover breakfast/lunch/snack (the stub LLM classifier deterministically assigns every fixture recipe to `lunch`/`main`, same known limitation the Phase F4 e2e rehearsal documented — worked around here via the override endpoint rather than the file-based rehearsal workaround, since this check only needed one working week, not a full suite run), regenerated the week, and marked prep complete to get a genuinely committed week — all via direct `curl` against the gateway, no frontend mutation path involved. Confirmed two things: (1) `curl`ing `GET /weeks/2026-09-07` directly showed the raw response has no `unlock` key at all — the exact shape `WeekResponse.unlock?` in `daycare.ts` assumes; (2) loaded the dashboard in a real browser (Nuxt dev server from this branch, `API_URL` pointed at the staging gateway) logged in as the built-in staging admin against that now-committed week — the locked-week notice ("This week is completed and locked. Completed on 9/3/2026...") renders with **no "Unlock Week" button**, exactly the "hidden when absent" behavior, and the browser console showed zero errors on that load. Screenshot: locked week, admin, no unlock action, sidecar on `main`.

**Not yet verified against `fm/mdp-unlock-week-endpoint` carrying real unlock code** — that branch had no commits beyond `main` as of this phase's authorship, so the safe/unsafe preview and unlock flows themselves (preview rendering, single-confirm safe path, forced+reason unsafe path, receipt) have no live endpoint to exercise yet; coverage for those paths is unit tests only (`DaycareUnlockDialog.test.ts`, `use-daycare.test.ts`) until the sidecar lane lands real code and this can be rechecked live against it.
